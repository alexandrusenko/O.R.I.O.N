...
class OrionConsoleUI:
    ...
    def render_boot(self, model_name: str) -> None:
        ...
        self.console.print("[bold white]Omni-Resourceful Intelligent Operations Network online.[/]")
        self.render_status(model_name=model_name, state="Idle")

    def render_user(self, text: str) -> None:
        self.console.print(Panel(text, title="User", border_style="gold1"))
    ...
    def render_status(self, model_name: str, state: str) -> None:
        ...
        table.add_row("[cyan]Time[/]", datetime.now().strftime("%H:%M:%S"))
        table.add_row("[cyan]Model[/]", model_name)
        table.add_row("[cyan]State[/]", state)
